...
class ReplayBuffer(object):
    ...
    def store_transiton(self,state,action,reward,state_neu, termianted):
        index = self.mem_cntr & self.mem_size
        self.state_memory[index] = state
        self.action_memory[index]=action
        self.new_state_memory[index]=state_neu
        self.terminal_memory[index] = 1 - termianted
        self.mem_cntr +=1
...
class CriticNetwork(nn.Module):
    def __init__(self, beta, input_dims, fc1_dims, fc2_dims, n_actions, name, chkpt_dir="tmp/ddpg"):
        ...
        self.optimizer = optimum.Adadelta(self.parameters(), lr = beta)
...
class ActorNetwork(nn.Module):
        def __init__(self, alpha, input_dims,fc1_dims ,fc2_dims, n_actions, name, chkpt_dir ='tmp/ddpg'):
            ...
            self.optimizer = optimum.Adadelta(self.parameters(), lr = alpha)
...
class Agent(object):
    def __init__(self, alpha, beta, input_dims, tau , env, gamma = 0.99, n_actions =2, max_size = 1000000,layer1_size = 400, layer2_size =300, batch_size = 64):
        ...
        self.actor = ActorNetwork(alpha, input_dims, layer1_size, layer2_size,n_actions=n_actions, name ="TargetNetwork")
    ...
    def replay(self,state, action, reward, new_state, terminated):
        self.memory.store_transiton(state, action, reward, new_state, terminated)
    
    def learn(self):
        if self.memory.mem_cntr < self.batch_size:
            return 
        state, action ,reward, new_state , terminated = self.memory.sample_buffer(self.batch_size)
        state = T.tensor(state, dtype = T.float).to(self.critic.device)
        terminated = T.tensor(terminated, dtype = T.float).to(self.critic.device)
        ...
        for j in range(self.batch_size):
            target.append(reward[j] + self.gamma*critic_value[j]*terminated[j])
