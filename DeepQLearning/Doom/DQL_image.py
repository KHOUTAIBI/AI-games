...
class DeepQNetwork(nn.Module):
    def __init__(self, input_dims,n_actions):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(4,32,3,stride=4), nn.BatchNorm2d(32))
        self.conv2 = nn.Sequential(nn.Conv2d(32,32,3,stride=2), nn.BatchNorm2d(32))
        self.conv3 = nn.Sequential(nn.Conv2d(32,32,3,stride=2), nn.BatchNorm2d(32))
        self.conv4 = nn.Sequential(nn.Conv2d(32,64,3,stride=1), nn.BatchNorm2d(64))
    ...
    def forward(self, x):
        ...
        x = self.relu(self.conv4(x).flatten(start_dim=1))
        x = self.relu(self.fc1(x))
        return self.relu(self.fc2(x))

class Memory():
    def __init__(self,input_dims,max_mem_size = 10000):
...
def process_image(x):
    ...
    transform = transforms.Compose([
        ...
        transforms.Resize([30, 45], interpolation=transforms.InterpolationMode.NEAREST),  # Resizes the image
        x = x.expand(45,-1)
    ])
    x = transform(x)
